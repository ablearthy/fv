...
Let k'_inv := calc_k' m k.
Let m'_inv := calc_m' m k'.


Lemma k'_inv_is_positive : k'_inv > 0.
Proof.
  rewrite /k'_inv /calc_k' ltn_divRL ; last by exact: m_divisible.
  rewrite mul0n subn_gt0. by exact: m_sqr_ltn_N.
Qed.

Lemma k'_inv_has_upper_bound : k'_inv <= nsqrt + m'_inv.
Proof.
Admitted.

Lemma k'_inv_is_bounded : 0 < k'_inv <= nsqrt + m'_inv.
Proof.
  apply/andP. constructor. by exact: k'_inv_is_positive. by exact: k'_inv_has_upper_bound.
Qed.

Lemma m'_inv_is_bounded : 0 < m'_inv <= nsqrt.
Proof.
Admitted.

Lemma nsqrt_inv_is_bounded' : nsqrt < m'_inv + k'_inv.
Proof.
Admitted.

Lemma m'_inv_divisible_k'_inv : k'_inv %| (N - m'_inv ^ 2).
Proof.
Admitted.

Lemma step_vbase_inv_prf (b : vbase) : validBase (step_inv (val b)).
Proof.
  move: b => [b /[dup] /and4P [H1 H2 H3 H4] VB].
  apply/and4P ; split ; rewrite /step /=.
  - by apply: k'_inv_is_bounded.
  - by apply: m'_inv_is_bounded.
  - by apply: nsqrt_inv_is_bounded'.
  - by apply: m'_inv_divisible_k'_inv.
Qed.

Definition vstep_inv (b : vbase) : vbase := exist validBase (step_inv (val b)) (step_vbase_inv_prf b). *)